...
async function main(argv = process.argv.slice(2), fetchImpl = globalThis.fetch) {
  ...
  if (args.command === "auth" && args.subcommand === "status") { const state = await readState(); const scopes = Array.isArray(state?.scopes) ? state.scopes : String(state?.scopes || "").split(/\s+/).filter(Boolean); return state ? { status: "configured", scopes } : { status: "signed_out" }; }
